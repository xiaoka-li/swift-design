...
//  DesignCode
//
//  Created by xiaoka on 2022/10/24.
//
    ...
    var body: some View {
        TimelineView(.animation) { timeLine in
            ...
            Canvas { context, size in
                context.fill(path(in: CGRect(x: 0, y: 0, width: size.width, height: size.height), x: x, y: y), with: .linearGradient(Gradient(colors: [.pink, .blue]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: 400, y: 400)))
            }
...
//        BlobShape().frame(width: 400,height: 414)
...
func path(in rect: CGRect, x: Double, y: Double) -> Path {
    ...
    path.addCurve(to: CGPoint(x: 0.6355*width*y, y: height), control1: CGPoint(x: 0.92554*width*y, y: 0.77749*height), control2: CGPoint(x: 0.91864*width*y, y: height))
